...
export function CoachPanel({
  ...
  onBuildCoachPackage,
  clientImportText,
  onClientImportTextChange,
  onImportClientReport,
  clientSummary,
}: {
  isCoach: boolean;
  coachPackageText: string;
  onBuildCoachPackage: () => void;
  clientImportText: string;
  onClientImportTextChange: (text: string) => void;
  onImportClientReport: () => void;
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <Card className="rounded-2xl shadow-md">
        ...
        <CardContent className="space-y-3">
          <Button className="rounded-xl" onClick={onBuildCoachPackage}>
            Build coach package
          </Button>
          <Textarea
      ...
      <Card className="rounded-2xl shadow-md">
        ...
        <CardContent className="space-y-3">
          <Textarea
            ...
            onChange={(e) => onClientImportTextChange(e.target.value)}
            placeholder="Paste client export JSON here..."
          />
          <Button className="rounded-xl" onClick={onImportClientReport}>
            Load client report
          </Button>
          {clientSummary ? (
